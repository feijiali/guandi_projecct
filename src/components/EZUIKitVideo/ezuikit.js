...
var EZUIKitPlayer = function EZUIKitPlayer(params) {
  ...
  addJs(jqueryJS, function () {
    addJs(layerJs, function () {
      ...
      if (
        matchFooterOpt().footerContainer ||
        _this.opt.plugin.indexOf("talk") !== -1
      ) {
        ...
        if (matchFooterOpt().footerContainer) {
          ...
          if (matchFooterOpt().hdModule || matchFooterOpt().fullScreenModule) {
            ...
            if (matchFooterOpt().hdModule) {
              ...
              hdDom.innerHTML =
                _this.opt.url.indexOf(".hd") === -1 ? "标清" : "高清";
              ...
              hdDom.onclick = function () {
                ...
                iframe.src =
                  ...
                  _this.opt.url.replace(".hd.live", ".live") +
                  "&autoplay=1&audio=" +
                  _this.opt.audio +
                  "&accessToken=" +
                  _this.opt.accessToken +
                  "&templete=" +
                  0;
              };
          ...
          if (matchFooterOpt().broadcastModule) {
            ...
            function randerVoliceList(data) {
              ...
              if (data && data.length > 0) {
                for (var i = 0; i < data.length; i++) {
                  ...
                  voiceItem.innerHTML =
                    "<li class='voice-item' id='voice-item-" +
                    i +
                    "' data-time=" +
                    (data[i]["duration"] || 20) +
                    " data-url=" +
                    data[i]["fileUrl"] +
                    ">" +
                    (data[i]["voiceName"].length > 10
                      ? data[i]["voiceName"].substr(0, 10) + "..."
                      : data[i]["voiceName"]) +
                    "</li>";
                  document
                    .getElementsByClassName("voice-list-ul")[0]
                    .append(voiceItem); // "<li class='voice-item' id='voice-item-" + i + "' data-time=" + (data[i]["duration"] || 20) + " data-url=" + data[i]["fileUrl"] + ">" + (data[i]["voiceName"].length > 10 ? (data[i]["voiceName"].substr(0, 10) + "...") : data[i]["voiceName"]) + "</li>";
                ...
                if (data.length === EZUIKit.state.pageSize) {
                  document.getElementById("voice-list-end").innerHTML =
                    "向下滚动加载更多";
                } else {
                  document.getElementById("voice-list-end").innerHTML =
                    "没有更多数据了";
            ...
            document.getElementById("voice-list").onscroll = function (e) {
              ...
              console.log(
                ...
                sum,
                this.scrollTop,
                document.getElementById("voice-list").clientHeight
              );
            ...
            document.getElementById("stopSpeak").onmouseup = function () {
              ...
              function wavUpload() {
                try {
                  ...
                  recorder &&
                    recorder.exportWAV(function (wav_file) {
          ...
          function countTime(type, start = 0) {
            ...
            if (type === "add") {
              ...
              EZUIKit.state.countTimer = setInterval(function () {
                ++i;
                document.getElementById("time-area").innerHTML = formatSeconds(
                  i
                );
              }, 1000);
            } else if (type === "sub") {
          ...
          function formatSeconds(value) {
            ...
            secondV =
              parseInt(theTime) > 9
                ...
                : "0" + parseInt(theTime);
            minV =
              parseInt(middle) > 9 ? parseInt(middle) : "0" + parseInt(middle);
  ...
  function message(event) {
    ...
    console.log("EZUIKitPlayer收到反馈", event.data.type);
    var id = _this.opt.id;
    if (event.data.type) {
  ...
  window.addEventListener("message", _this.message);
}; // 播放相关API
...
EZUIKitPlayer.prototype.destroy = function () {
  console.log("清楚信息监听", this.message);
