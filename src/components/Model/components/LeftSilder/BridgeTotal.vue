      ...
      <vScroll>
        <ul>
          <li class="p-1 text-white" v-for="(item, index) in list" :key="index">
            <p>{{ types[item.build_type] }}</p>
            <div class="d-flex py-1 px-2">
              <canvas
                class="lt canvas vtm"
                :id="'bridge-canvas-' + index"
                width="64"
                height="64"
              ></canvas>
              <div class="d-felx flex-column align-content-center p-1">
                <div class="flex-fill">
                  ...
                  <span class="text-hover-primary" style="max-width: 56px">{{
                    $utils.float_point(item.plan_number, 2)
                  }}</span>
                  <span>{{ units[item.build_type] }}</span>
                </div>
                <div class="flex-fill">
                  实际进度
                  <span class="red" style="max-width: 56px">{{
                    $utils.float_point(item.build_number, 2)
                  }}</span>
                  <span>{{ units[item.build_type] }}</span>
                </div>
              </div>
...
export default {
  ...
  methods: {
    ...
    getTotalData() {
      ...
      api
        .bridgeProgressDetail(params)
        .then(({ result }) => {
          if (result[0]) {
            this.list = result[0].balance.map((item) => ({
              ...item,
              percent: Number(item.ration.split("%")[0]),
            }));
            this.canvasFun(this.list, "bridge-canvas-");
          } else {
            this.list = [];
          }
        })
