    ...
    <div class="rounded-1 bg-white flex-fill p-2">
      <n-scroll>
        <el-form ref="ruleform" class="el-form-box" :model="ruleform">
          <el-form-item class="form-item-long-small must" label="考核名称：">
            <el-input
              class="input-m"
              v-model="ruleform.examine_name"
              placeholder="请输入"
            ></el-input>
          </el-form-item>
          ...
          <el-form-item class="form-item-long-small must" label="考核类型：">
            <el-select
              v-model="ruleform.examine_type"
              placeholder="请选择/输入"
              filterable
              allow-create
            >
          ...
          <el-form-item class="form-item-long-small must" label="考核日期：">
            <el-date-picker
              :clearable="false"
              v-model="ruleform.examine_time"
              format="yyyy-MM-dd"
          ...
          <el-form-item class="form-item-long must" label="考核对象：">
          ...
          <el-form-item class="form-item-long" label="考核内容：">
            <el-input
              v-model="ruleform.examine_content"
              type="textarea"
              placeholder="请输入"
              rows="6"
...
export default {
  ...
  data() {
    return {
      ruleform: {
        examine_id: null,
        organ_id_union: null,
        examine_time: this.$utils.getCurrentTime(),
        examine_video: "",
        examine_pic: "",
        examine_file: "",
        examine_name: "",
  ...
  methods: {
    ...
    choosePersonNodes(nodes) {
      ...
      this.personIDsArr = this.examine_id ? [arr] : arr;
    ...
    async querySafeExamine() {
      let data = await api.querySafeExamine({
      ...
      this.personIDsArr = info.examine_target.split(",").map(Number);
    ...
    // 确认提交
    async submitFun() {
      this.ruleform.examine_target = this.personIDsArr.length
        ? this.personIDsArr.join(",")
        : "";
      // 先判断是否填写必填信息
      if (this.$utils.ruleFormTip(this, this.formValidation, this.ruleform)) {
        let file_obj = { uploadFileModule: "SafeExamine" };
        this.ruleform.examine_pic = await this.$refs.pic_upload.submitFun(
          false,
          file_obj
        );
        this.ruleform.examine_video = await this.$refs.video_upload.submitFun(
          false,
          file_obj
        );
        this.ruleform.examine_file = await this.$refs.file_upload.submitFun(
          false,
          file_obj
        );
        this.submitAllData();
    ...
    async submitAllData() {
      ...
      let res = await api[
        `${this.examine_id ? "updateSafeExamine" : "uploadSafeExamine"}`
      ](subdata);
      this.$tip({
        isTip: false,
        closeTime: 1000,
