      ...
      <n-scroll>
        <el-form ref="ruleform" class="el-form-box" :model="ruleform">
          ...
          <el-form-item
            ...
            v-if="ruleform.inspection_project_type == 1"
            label="工程名称："
          >
            <el-select
          ...
          <el-form-item
            class="form-item-long-small must"
            v-if="ruleform.inspection_project_type == 2"
            label="工程名称："
          >
            <el-select
              v-model="ruleform.inspection_project_name"
            ...
            >
              <el-option
                ...
                :key="item.unit_project_id"
                :label="item.unit_project_name"
                :value="item.unit_project_name"
              ></el-option>
            </el-select>
          </el-form-item>
          <el-form-item
...
export default {
  ...
  async mounted() {
    ...
    if (this.type == 1) {
      await this.querySafeDailyInspection();
    } else {
  ...
  methods: {
    ...
    async queryBridgeUnitProject() {
      let data = await api.queryBridgeUnitProject({
        page: -1,
        organ_node: this.$organ_id_en_or_pro(this),
      });
    ...
    async changeTunnel(data) {
      let res = await api.queryTunnelUnitAndSubUnit({
        page: -1,
        organ_node: this.$organ_id_en_or_pro(this),
        unit_proj_name: data,
      });
      this.Mileage =
        res.result[0].unit_proj_pile_prefix +
        res.result[0].unit_proj_pile_start_string +
        "—" +
        res.result[0].unit_proj_pile_prefix +
        res.result[0].unit_proj_pile_end_string;
      this.ruleform.inspection_project_pile_prefix =
        res.result[0].unit_proj_pile_prefix;
    },
    //获取桥梁的桩号范围
    ...
    async querySafeDailyInspection() {
      let data = await api.querySafeDailyInspection({
        page: 0,
        page_size: 1,
        daily_inspection_status: 0,
        organ_node: this.$store.getters.organID,
      ...
      let data2 = await api.querySafeDailyInspection({
    ...
    async submitFun() {
      ...
      if (this.$utils.ruleFormTip(this, this.formValidation, this.ruleform)) {
        let file_obj = { uploadFileModule: "SafeDailyInspection" };
