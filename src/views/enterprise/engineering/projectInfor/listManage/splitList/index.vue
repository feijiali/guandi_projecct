...
export default {
  ...
  mounted() {
    ...
    this.listQuery.proj_detail_split_organ = Number(this.$route.query.id);
    this.$bus.$on("select_list", (item) => {
      if (item.tag == 0 || item.tag == 1 || item.tag == 2) {
        this.tableData = [];
        this.showbutton = false;
      } else {
        this.listQuery.proj_detail_split_organ = item.tag;
        this.getList();
        this.showbutton = true;
      }
    });
    if (
      this.listQuery.proj_detail_split_organ == 0 ||
      this.listQuery.proj_detail_split_organ == 1 ||
      this.listQuery.proj_detail_split_organ == 2
    ) {
      this.tableData = [];
      this.showbutton = false;
    } else {
  ...
  methods: {
    beforeClose() {
      this.params.proj_detail_split_organ = null;
    ...
    Comdetail(id) {
      this.linkTo("enterprise_engineering_splitList_info", {
