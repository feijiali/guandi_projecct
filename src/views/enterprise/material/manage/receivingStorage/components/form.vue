  ...
  <div class="w-100">
    <el-form class="el-form-box"
      ...
      <el-form-item class="form-item-long-small">
        <a :href="GLOBAL.baseurl + downLoadURL"
           download="合同计量模版.xlsx">
          <n-button :btnText="'下载模板'"
                    :width="'120px'"
                    :colorBtn="'blue'"></n-button>
        </a>
        <el-upload v-permission="['ProjectReportEngineeringMain-upload']"
                   class="
            upload
            download-item
            position-relative
      ...
      <material-table v-model="list"
                      ...
                      @inputChange="calFun"></material-table>
    </el-form>
  </div>
</template>
...
export default {
  ...
  data () {
    return {
      ...
      headers: [
        ...
        { key: "name", name: "材料名称及工程部位", width: 120, placeholder: "搜索/选择/请输入", type: 2, historyArr: [] },
        { key: "unit", name: "单位", width: 80, placeholder: "自动获取/请输入", type: 1 },
        { key: "num", name: "图纸数量", width: 80, placeholder: "自动获取/请输入" },
        { key: "allnum", name: "累计完成设计量", width: 80, placeholder: "自动获取/请输入" },
        { key: "allreal", name: "累计完成实际量", width: 80, placeholder: "自动获取/请输入" },
        { key: "a", name: "累计盈亏(盈-亏+)", width: 80, placeholder: "自动计算/请输入" },
        { key: "b", name: "累计损耗率(%)", width: 80, placeholder: "自动计算/请输入" },
        { key: "c", name: "本期施工设计量", placeholder: "请输入" },
        { key: "d", name: "本期施工实际量", placeholder: "请输入" },
        { key: "e", name: "本期盈亏(盈-亏+)", width: 80, placeholder: "自动计算/请输入" },
        { key: "f", name: "本期损耗率(%)", width: 80, placeholder: "自动计算/请输入" },
        { key: "remark", name: "备注(损耗分析)", placeholder: "请输入", type: 1 }
      ],
      ruleforms: {
  ...
  methods: {
    ...
    chooseEngineeringName (key, value, index) {
      ...
      this.list[index].allnum = 1.00
      this.list[index].allreal = 2.00
      profitCalc(this.list[index])
    },
    calFun (key, value, index) {

      (key == "allnum" || key == 'allreal') && this.cumulativeCalc(this.list[index]);
