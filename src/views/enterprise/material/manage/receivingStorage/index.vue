        ...
        <div class="top-select-box mt-1 bg-white">
          ...
          <div>
            <n-button class="ml-2"
                      :btnText="'新增'"
                      @buttonClick="
              $router.linkTo('enterprise_material2_material_wastage_handle')
            ">
              <template v-slot:button-slot>
                <i class="iconfont btn-btn_detail_plus_01 mr-1"></i>
              </template>
            </n-button>
            <n-button v-permission="['ProjectReportEngineeringMain-export']"
                      class="ml-2"
                      :btnText="'导出'"
                      @buttonClick="
                $utils.handleExport(
...
export default {
  ...
  methods: {
    ...
    async getTreeData () {
      ...
      let url = process.env.NODE_ENV == 'development' ? "../../.." : this.GLOBAL.baseurl + "/web";

      this.src = url + "/h5/material/wastage.html?id=" +
        (this.tableData[0] ? this.tableData[0].engineering_main_id : 1);
