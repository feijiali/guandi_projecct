 ...
 * @LastEditors: 邓易
 * @LastEditTime: 2021-03-31 14:02:24
-->
<template>
      ...
      <n-scroll>
        <div class="">
          ...
          <n-button
            ...
            :btnText="'转发'"
            colorBtn="purple"
            @buttonClick="handleAdd('_forwarding')"
          >
            <template v-slot:button-slot>
        ...
        <div class="mb-1 mt-2">
          ...
          <div class="font-14 mt-05">
            发件人：{{
              info.send_name + (info.send_email ? "—" + info.send_email : "")
            }}
          </div>
        ...
        <p class="mt-1 ml-1" v-if="emailAccount.length">
          已读人员：<span v-for="(item, index) in emailAccount" :key="index"
            >{{ item.account_name
            }}{{ index != emailAccount.length - 1 ? "，" : "" }}</span
          >
        </p>
...
export default {
  ...
  async created() {
    ...
    await this.QueryOaEmail();
    this.QueryOaEmailAccount();
  },
  methods: {
    ...
    async QueryOaEmailAccount() {
      let { result } = await api.QueryOaEmailAccount({
        email_id: this.edit_id,
      });
      this.emailAccount = result;
    },
    handleAdd(name_type) {
      let name = "enterprise_oa_email_sented" + name_type;
