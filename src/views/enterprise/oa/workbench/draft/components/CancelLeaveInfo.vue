<template>
  <div class="rounded-1 bg-white flex-fill">
    ...
    <el-table
      ...
      <el-table-column label="销假事由" prop="leave_cancel"></el-table-column>
      <el-table-column label="销假时间" prop="leave_start_time_string">
        <template slot-scope="scope">
          {{ scope.row.leave_cancel_start_date }}
          {{ scope.row.leave_cancel_start_time }} 至
          {{ scope.row.leave_cancel_end_date }}
          {{ scope.row.leave_cancel_end_time }}
        </template>
      </el-table-column>
  ...
  watch: {
    tableData() {
      ...
      this.tableData.forEach((item) => {
        console.log(item.draft_content);
