 ...
 * @LastEditors: 邓易
 * @LastEditTime: 2021-01-25 16:31:30
-->
<template>
  <div class="w-100">
    ...
    <template v-if="infoData">
      ...
      <el-table
        ...
        <el-table-column
          label="设备名称"
          prop="apply_detail_name"
        ></el-table-column>
        <el-table-column
          label="设备型号"
          prop="apply_detail_spec"
        ></el-table-column>
        <el-table-column
          label="设备品牌"
          prop="apply_detail_bound"
        ></el-table-column>
        <el-table-column
          label="预算单价(元)"
          prop="apply_detail_price"
        ></el-table-column>
        <el-table-column
          label="需求数量"
          prop="apply_detail_num"
        ></el-table-column>
        <el-table-column
          label="金额(元)"
          prop="apply_detail_money"
        ></el-table-column>
        <el-table-column label="是否加急">
          <span slot-scope="scope">
            {{ ["", "加急", "不加急"][scope.row.apply_detail_is_urgent] }}
          </span>
        </el-table-column>
        <el-table-column
          label="计划使用时间"
          prop="apply_detail_plan_use_time"
          width="150"
        ></el-table-column>
      </el-table>
    </template>
...
export default {
  ...
  methods: {
    ...
    async infoFun(id) {
      let { result } = await api.QueryApply({
        apply_id: id,
      });
      if (!result[0]) {
        this.$tip({
