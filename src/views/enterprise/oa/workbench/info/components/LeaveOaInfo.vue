<template>
  <div class="w-100 d-flex h-100">
    <n-scroll>
      <div class="w-50">
        <nInfoTitle :title="'请假详情'" class="mb-1" />
        <nInfoItem
      ...
      <div class="w-50" v-if="infoData.leave_cancel_revise_id_union">
        <nApprovalProcess
          :title="'销假'"
    ...
    async infoFun(id) {
      ...
      if (!result[0]) {
        ...
        if (this.infoData.leave_cancel_mark == 1) {
          this.$emit("changeTitle", "请假申请详情");
        }
      }
      if (this.infoData.leave_cancel_revise_id_union) {
