  ...
  <div class="w-100 h-100 table-list-wrap">
    ...
    <div class="rounded-1 bg-white flex-fill">
      <div class="top-select-box">
        ...
        <div>
          <n-button
            v-permission="['UnionPartySubsidyCompensate-upload']"
            :btnText="'草稿箱'"
            class="mr-2"
            @buttonClick="
              $router.linkTo('enterprise_oa_workbench_draft', {
                name: 'UnionPartySubsidyCompensate',
              })
            "
          >
          ...
          <n-button
            v-permission="['UnionPartySubsidyCompensate-delete']"
            :btnText="'批量删除'"
            @buttonClick="
          ...
          <n-button
            v-permission="['UnionPartySubsidyCompensate-export']"
            class="ml-2"
            :btnText="'批量导出'"
            @buttonClick="
...
export default {
  ...
  data() {
    return {
      ...
      listQuery: {
        ...
        subsidy_type: 4, //2:金秋助学3:生病住院慰问4:补偿申请
  ...
  activated() {
    this.listQuery.organ_node = this.$route.query.organ_id
      ? parseInt(this.$route.query.organ_id)
      : this.$organ_id_en_or_pro(this);
    if (hasPermissionArr(["UnionPartySubsidyCompensate-query"])) {
      //如果勾选查看权限，则能看所有的
  ...
  methods: {
    ...
    rowclick(row) {
      ...
      this.$router.linkTo("enterprise_oa_workbench_info", {
        t: 4,
        types: "UnionPartySubsidyCompensate",
