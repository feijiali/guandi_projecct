  ...
  <div class="w-100 h-100 table-list-wrap">
    ...
    <div class="rounded-1 bg-white flex-fill">
      <div class="top-select-box">
        ...
        <div>
          <n-button
            v-permission="['UnionPartyHardWorker-upload']"
            :btnText="'草稿箱'"
            class="mr-2"
            @buttonClick="
              $router.linkTo('enterprise_oa_workbench_draft', {
                name: 'UnionPartyHardWorker',
              })
            "
          >
      ...
      <el-table
        ...
        ></el-table-column>
        <el-table-column label="申请时间" prop="subsidy_sub_time">
          <template slot-scope="scope">
            <span>{{ scope.row.hard_worker_sub_time.slice(0, 10) }}</span>
          </template>
        </el-table-column>
...
export default {
  ...
  data() {
    return {
      postURL: "queryUnionPartyHardWorker", //表格请求接口
      listQuery: {
        //列表查询参数
        page: 0,
        page_size: 20,
  ...
  activated() {
    this.listQuery.organ_node = this.$route.query.organ_id
      ? parseInt(this.$route.query.organ_id)
      : this.$organ_id_en_or_pro(this);
    if (hasPermissionArr(["UnionPartyHardWorker-query"])) {
      //如果勾选查看权限，则能看所有的
