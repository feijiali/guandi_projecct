  ...
  <div class="w-100 h-100 table-list-wrap">
    ...
    <div class="rounded-1 bg-white flex-fill">
      <div class="top-select-box">
        ...
        <div>
          <n-button
            v-permission="['StaffMobilize-upload']"
            :btnText="'草稿箱'"
            class="mr-2"
            @buttonClick="
          ...
          <n-button
            v-permission="['StaffMobilize-delete']"
            :btnText="'批量删除'"
            @buttonClick="
              deleteFun(true, '', 'mobilize_id', 'delStaffMobilize')
            "
          >
            <template v-slot:button-slot>
              <i class="iconfont btn-btn_list_trash trash-icon-btn mr-1"></i>
          ...
          <n-button
            v-permission="['StaffMobilize-export']"
            class="ml-2"
            :btnText="'批量导出'"
            @buttonClick="
      ...
      <el-table
        ...
        <el-table-column
          label="调出部门"
          prop="before_organ_name"
        ></el-table-column>
        <el-table-column
          label="调入部门"
          prop="after_organ_name"
        ></el-table-column>
        <el-table-column
          label="调离日期"
          prop="mobilize_time"
        ...
        <el-table-column label="操作" header-align="center" width="80">
          <template slot-scope="scope">
            <i
              v-permission="['StaffMobilize-delete']"
              @click.stop="
                deleteFun(false, scope.row, 'mobilize_id', 'delStaffMobilize')
              "
              class="
                iconfont
                btn-btn_list_trash
...
export default {
  ...
  data() {
    return {
      postURL: "queryStaffMobilize", //表格请求接口
      listQuery: {
        //列表查询参数
        page: 0,
        page_size: 20,
  ...
  activated() {
    if (hasPermissionArr(["StaffMobilize-query"])) {
      //如果勾选查看权限，则能看所有的
