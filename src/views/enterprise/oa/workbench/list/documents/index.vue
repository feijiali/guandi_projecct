  ...
  <div class="w-100 h-100 table-list-wrap">
    ...
    <div class="rounded-1 bg-white flex-fill">
      <div class="top-select-box">
        ...
        <div>
          <n-button
            v-permission="['OAInternalDocument-upload']"
            :btnText="'草稿箱'"
            class="mr-2"
            @buttonClick="
              $router.linkTo('enterprise_oa_workbench_draft', {
                name: 'OAInternalDocument',
              })
            "
          >
          ...
          <n-button
            v-permission="['OAInternalDocument-delete']"
            :btnText="'批量删除'"
            @buttonClick="
              deleteFun(
                true,
          ...
          <n-button
            v-permission="['OAInternalDocument-export']"
            class="ml-2"
            :btnText="'批量导出'"
            @buttonClick="
              handleExportFromSelected(
                'ExportOAInternalDocument',
                'internal_document_id',
                tableSelectList.length
                  ? { account_id: listQuery.account_id }
                  : listQuery,
                '内部文件表'
              )
            "
      ...
      <el-table
        ...
        <el-table-column
          label="文件名称"
          prop="internal_document_name"
        ></el-table-column>
        <el-table-column
          label="上传日期"
          prop="internal_document_create_time"
        ></el-table-column>
        <el-table-column
          label="备注"
          prop="internal_document_remark"
          width="200"
        ></el-table-column>
        <el-table-column label="上传人" prop="account_name"></el-table-column>
...
export default {
  ...
  data() {
    return {
      postURL: "QueryOAInternalDocumentList", //表格请求接口
      listQuery: {
        //列表查询参数
        page: 0,
        page_size: 20,
      ...
      dateRange: [],
      keyID: "internal_document_id",
      getListBool: false,
    };
  },
  activated() {
    if (hasPermissionArr(["OAInternalDocument-query"])) {
      //如果勾选查看权限，则能看所有的
