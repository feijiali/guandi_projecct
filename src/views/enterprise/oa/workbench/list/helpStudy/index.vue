  ...
  <div class="w-100 h-100 table-list-wrap">
    ...
    <div class="rounded-1 bg-white flex-fill">
      <div class="top-select-box">
        ...
        <div>
          <n-button
            v-permission="['UnionPartySubsidyStudy-upload']"
            :btnText="'草稿箱'"
            class="mr-2"
            @buttonClick="
              $router.linkTo('enterprise_oa_workbench_draft', {
                name: 'UnionPartySubsidyStudy',
              })
            "
          >
...
export default {
  ...
  data() {
    return {
      ...
      listQuery: {
        ...
        subsidy_type: 2, //2:金秋助学3:生病住院慰问4:补偿申请
  ...
  activated() {
    this.listQuery.organ_node = this.$route.query.organ_id
      ? parseInt(this.$route.query.organ_id)
      : this.$organ_id_en_or_pro(this);
    if (hasPermissionArr(["UnionPartySubsidyStudy-query"])) {
      //如果勾选查看权限，则能看所有的
  ...
  methods: {
    ...
    // 跳转新增页面
    async linkTo() {
      let res = await this.queryUnionPartyHardWorkerList();
      if (!res.result[0]) {
        this.$tip({
          isTip: true,
          message: "抱歉，困难职工才能申请！",
      ...
      this.$router.linkTo("enterprise_oa_workbench_apply", {
    ...
    async queryUnionPartyHardWorkerList() {
      let res = await api.queryUnionPartyHardWorkerList({
        ...
        hard_worker_hard_state_end: 5,
        hard_worker_account_id_union: Number(this.$store.getters.account_id),
        page: -1,
