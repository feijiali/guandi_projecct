  ...
  <div class="w-100 h-100 table-list-wrap">
    ...
    <div class="rounded-1 bg-white flex-fill">
      <div class="top-select-box">
        ...
        <div>
          <n-button
            v-permission="['OAOvertime-upload']"
            :btnText="'草稿箱'"
            class="mr-2"
            @buttonClick="
          ...
          <n-button
            ...
            :btnText="'批量删除'"
            @buttonClick="deleteFun(true, '', 'overtime_id', 'DelOAOvertime')"
          >
            <template v-slot:button-slot>
              <i class="iconfont btn-btn_list_trash trash-icon-btn mr-1"></i>
            </template>
          ...
          <n-button
            v-permission="['OAOvertime-export']"
            class="ml-2"
            :btnText="'批量导出'"
            @buttonClick="
        ...
        <el-table-column label="操作" header-align="center" width="80">
          <template slot-scope="scope">
            <i
              v-permission="['OAOvertime-delete']"
              @click.stop="
                deleteFun(false, scope.row, 'overtime_id', 'DelOAOvertime')
              "
              class="
                iconfont
                btn-btn_list_trash
...
export default {
  ...
  data() {
    return {
      postURL: "QueryOAOvertimeList", //表格请求接口
      listQuery: {
        //列表查询参数
        page: 0,
        page_size: 20,
  ...
  activated() {
    if (hasPermissionArr(["OAOvertime-query"])) {
      //如果勾选查看权限，则能看所有的
  ...
  methods: {
    ...
    getSelectValue(node) {
      ...
      //     message: `新增人员只能选择部门`,
      //   });
      //   this.listQuery.organ_node = null;
    ...
    searchFun(val) {
      this.listQuery.overtime_search = val;
      this.refreshData();
    },
    // 筛选条件后重新请求数据
    refreshData() {
