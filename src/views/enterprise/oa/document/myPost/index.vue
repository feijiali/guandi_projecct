 ...
 * @LastEditors: 邓易
 * @LastEditTime: 2021-03-31 14:10:25
-->
<!--
 * @Autor: 邓易
 * @Date: 2021-03-03 10:24:51
  ...
  <div class="bg-white w-100 h-100 rounded-1 p-1">
    ...
    <el-table
      ...
      <el-table-column label="操作" width="140">
        <template slot-scope="{ row }">
          <span
            v-if="undoArr.indexOf(row.revise_info_state) == -1"
            class="main font-14"
            @click.stop="undo(row)"
            >撤销</span
          >
          <i
            v-if="
              !row.revise_account_reviewed_account_ids &&
              row.revise_info_state != 5
            "
            class="iconfont btn-btn_list_edit edit-icon-btn ml-1"
            @click.stop="handleAdd(row.receive_file_id)"
          ></i>
          <i
            v-if="
              (row.revise_info_state == 2 &&
                !row.revise_account_reviewed_account_ids) ||
              row.revise_info_state == 5 ||
              row.revise_info_state == 4
            "
  ...
  methods: {
    ...
    undo(row) {
      this.linkTo("enterprise_oa_document_myPost_undo", {
        reverse_id: row.receive_approve_id_un,
        title: row.receive_title,
      });
    },
    rowclick(row) {
      this.linkTo("enterprise_oa_document_myPost_info", {
    ...
    handleAdd(id) {
      this.linkTo("enterprise_oa_document_myPost_edit", { id: id });
    },
    linkTo(name, query) {
