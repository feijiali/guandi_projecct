  ...
  <div class="container-all">
    ...
    <div class="container-div">
        ...
        <div>
          ...
          <div class="d-inline-block ml-2">
            ...
            >
              ...
              <el-option label="资料未补全" :value="5"></el-option>
              <el-option label="资料已补全" :value="6"></el-option>
            </el-select>
          </div>
          <n-search
      ...
      <el-table
        ...
        <el-table-column
          label="确认人"
          prop="progress_account_name"
        ></el-table-column>
        <el-table-column
          label="确认发展对象时间"
          prop="pd_progress_time"
          width="160"
        ></el-table-column>
        <el-table-column label="资料是否齐全">
          <template slot-scope="scope">
            <span>{{ scope.row.pd_states == 5 ? "否" : "是" }}</span>
          </template>
        </el-table-column>
        <el-table-column label="确认状态" width="150">
          <template slot-scope="scope">
            <div @click.stop>
              <span :class="colorArr[scope.row.pd_states]">{{
                pdStateArr[scope.row.pd_states]
              }}</span>
            </div>
          </template>
        </el-table-column>
        <el-table-column label="操作" width="260" align="center">
          <template slot-scope="scope">
            <div @click.stop>
              <nStatusButton
                v-permission="['UnionPartyPersonnelDevelop-getset']"
                @buttonClick="scope.row.pd_states == 6 && changeFun(scope.row)"
                :changeColor="scope.row.pd_states == 5 ? '#D6DAE4' : '#fd5571'"
                :btnText="scope.row.pd_states == 5 ? '预备党员' : '转正'"
              />
              <nStatusButton
                class="ml-2"
                v-permission="['UnionPartyPersonnelDevelop-getset']"
                @buttonClick="
                  scope.row.pd_states == 5 &&
                    linkTo(
                      'enterprise_wisdom_party_development_prepartList_handle',
                      { id: scope.row.personnel_develop_id }
                    )
                "
                :changeColor="scope.row.pd_states == 5 ? '#fd5571' : '#D6DAE4'"
                :btnText="'补全资料'"
              />
            </div>
          </template>
...
export default {
  ...
  methods: {
    ...
    linkTo(name, query) {
      if (!hasPermissionArr(["UnionPartyPersonnelDevelop-getset"])) {
        return;
      }
