<template>
  <div class="container-all">
    ...
    <Detail ref="DetailDialog" :infoData="infoData" :types="1"></Detail>

    <div class="rounded-1 bg-secondary">
      <nBackTop :title="'金秋助学公示'" />
    </div>
    <div class="container-div">
      <div class="top-select-box">
        ...
        <div>
          ...
          <n-button
            v-permission="['UnionPartySubsidyStudy-export']"
            class="ml-2"
            :btnText="'导出'"
            @buttonClick="
              $utils.handleExport(
      ...
      <el-table
        ...
        ></el-table-column>
        <el-table-column label="公示天数" prop="hard_worker_tel">
          <template slot-scope="scope">
            <span :class="colorArr[scope.row.subsidy_state]">
              {{ scope.row.publicity_day }}
            </span>
          </template>
        </el-table-column>
        <el-table-column label="异议状态" prop="subsidy_publicity_state_string">
          <template slot-scope="{ row }">
            <span
              class="status-btn"
              :style="{
                backgroundColor: backgroundColorArr[row.subsidy_state],
                color: colorArr[row.subsidy_state],
              }"
              >{{ row.subsidy_publicity_state_string }}</span
            >
          </template>
        </el-table-column>
          ...
          <template slot-scope="scope">
            <div class="txc">
              <div class="d-inline-block" @click.stop>
                <nStatusButton
                  v-if="
                    scope.row.subsidy_state == 2 && scope.row.publicity_day < 5
                  "
              ...
              <div class="d-inline-block" @click.stop>
                <nStatusButton
                  v-permission="['UnionPartySubsidyStudy-confirm']"
                  v-if="
                    scope.row.subsidy_state == 3 ||
                    (scope.row.subsidy_state == 2 &&
...
export default {
  ...
  methods: {
    ...
    async infoClick(row) {
      ...
      /// 查询工会管理公示
      await this.queryUnionPartyPublicity(row.subsidy_id);
      this.$nextTick(async () => {
        await this.$refs.DetailDialog.$refs.helpStudyInfo.infoFun(
          row.subsidy_id
        );
        this.$nextTick(() => {
          this.$set(
            ...
            this.$refs.DetailDialog.$refs.helpStudyInfo.infoData
              .subsidy_publicity_remark
          );
        });
      });
    ...
    changeState(val) {
      ...
      this.listQuery.subsidy_state_start = startEndArr[val][0];
      this.listQuery.subsidy_state_end = startEndArr[val][1];
      this.refreshData();
    },
    // 筛选条件后重新请求数据
    refreshData() {
