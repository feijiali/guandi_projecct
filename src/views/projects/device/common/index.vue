  ...
  <div class="container-all">
    ...
    <div class="container-div">
      <div class="top-select-box">
        <div>
          <div class="d-inline-block">
            ...
            >
              ...
              <el-option label="未检养" :value="3"></el-option>
              <el-option label="检养预警" :value="2"></el-option>
        ...
        <div>
          <n-button
            v-permission="['EquipmentImportExport_TJSTC-export']"
            class="ml-2"
            :btnText="'导出'"
            @buttonClick="
              $utils.handleExport(
          ...
          <n-button
            v-permission="['EquipmentDevice_TJSTC-upload']"
            :btnText="'新增'"
            class="ml-2"
      ...
      <el-table
        ...
        <el-table-column label="当前状态" prop="general_device_state">
          <template slot-scope="scope">
            <span
              :class="[
                scope.row.general_device_state == '已检养'
                  ? 'blue'
                  : scope.row.general_device_state == '检养预警'
                  ? 'pink'
                  : scope.row.general_device_state == '正常'
                  ? 'green'
                  : 'grey2',
              ]"
              >{{ scope.row.general_device_state }}</span
            >
          </template>
        </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      listQuery: {
        ...
        device_date_start: "",
        device_date_end: "",
        check_status: 0, //检养状态：0：全部 1：正常 2：逾期
        device_name: "",
  ...
  methods: {
    ...
    handleDelete(row) {
      this.confirm().then(() => {
        this.deleData.device_id = row.device_id;
        this.sureDelete("delEquipmentDevice", () => {
          this.getList();
        });
      });
    },
    bigImg(ind, data) {
      this.$bus.$emit("bigImg", {
        index: ind,
        imglist: data,
      });
    },
