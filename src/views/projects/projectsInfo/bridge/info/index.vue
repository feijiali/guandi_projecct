  ...
  <div class="container-all">
    <div class="container-div-s">
      <div class="top-select-box">
        ...
        <div>
          <div class="d-inline-block">
            <n-button 
            v-permission="['BridgeUnitProject-upload']"
              :btnText="'新增'"
              @buttonClick="
                $router.linkTo('project_projectsInfo_bridge_info_handle')
              "
            >
      ...
      >
        ...
        <el-table-column
          prop="unit_project_name"
          label="单位工程"
        ></el-table-column>
        <el-table-column prop="organ" label="项目工区">
          <template slot-scope="scope">
            {{ scope.row.list?'':scope.row.organ }}
          </template>
        </el-table-column>
        ...
        <el-table-column
          prop="unit_project_times_s"
          label="开工时间"
        ></el-table-column>
        <el-table-column
          prop="unit_project_times_e"
          label="完工时间"
        ></el-table-column>
        <el-table-column
        ...
        <el-table-column label="操作" header-align="center" width="240">
          <template slot-scope="scope">
            <i
              v-permission="['BridgeUnitProject-update']"
              class="iconfont btn-btn_list_edit edit-icon-btn"
              @click.stop="handleEdit(scope.row)"
            ></i>
            <i
              v-permission="['BridgeUnitProject-delete']"
...
export default {
  ...
  data() {
    return {
      postURL: "queryBridgeUnitProject", //表格请求接口
      listQuery: {
        page: 0,
        page_size: 20,
  ...
  methods: {
    ...
    handleEdit(row) {
      if (row.list) {
        this.$router.linkTo("project_projectsInfo_bridge_info_handle", {
          id: row.unit_project_id,
        });
      } else {
        let parentRow= this.tableData[row.parentIndex];
        this.$router.linkTo("project_projectsInfo_bridge_info_handleSubunits", {
          sub_id: row.unit_project_id,
          id:parentRow.unit_project_id,
        });
      }
    },
    // 新增桥梁子单位工程
    ...
    handleDelete(row) {
      this.confirm().then(() => {
        let url = row.list ? "delBridgeUnitProject" : "delBridgeSubUnit";
        let params = {};
        if (row.list) {
          params.unit_project_id = row.unit_project_id;
        } else {
          params.sub_unit_id = row.unit_project_id;
        }
        api[url](params).then(() => {
          this.getList();
        });
